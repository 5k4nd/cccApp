...
class loginForm(Form):
    login = TextField('Login', [Required("Veuillez renseigner votre login.")])
    ...
    def validate(self):
        if not Form.validate(self):
            return False
        return testLogin(self.login.data, self.password.data)
...
def login():
    ...
    if request.method == 'POST':
        if form.validate() == False:
            ...
            session['sid'] = form.login.data
            session['role'] = getUser(form.validate()).role
            print session['role']
            return redirect(url_for('index'))
